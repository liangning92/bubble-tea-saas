...
| 步骤 | 操作 | 要求 |
|------|------|------|
| 1. 学习 | 搜索竞品方案、官方文档、最佳实践 | 必须先完成才能进入下一步 |
| 2. 记录 | 将问题记入 `docs/issues/{category}-{number}.md` | 包含问题描述、根因、解决方案 |
| 3. 修复 | 动手修复 | 学习完成后才能动手 |

**禁止：** 跳过学习直接修复。违反者暂停工作，等用户指示。

### 构建必须经过用户确认（最高优先级）

**禁止在未获得用户明确授权的情况下执行以下任何操作：**

| 操作 | 说明 |
|------|------|
| `npm run build` / `npm run build:*` | 任何构建命令 |
| `electron-builder` | 打包桌面应用 |
| `gh workflow run` | 触发 GitHub Actions |
| `git push` | 推送到远程仓库（会触发 CI） |
...
| 序号 | 主题 | 内容 |
|------|------|------|
| 1 | Electron/GitHub Actions | CI/CD 优化、构建最佳实践 |
| 2 | React 性能优化 | 组件优化、性能调优 |
| 3 | TypeScript 最佳实践 | 类型安全、高级技巧 |
| 4 | 印尼市场 SaaS | 本地竞品、功能对比 |

学习成果保存在 `memory/learning/` 目录。
