...
function monthName(month: number, locale: string = 'id'): string {
...
export async function generateIncomeStatementExcel(storeId: string, month: number, year: number, includeDepreciation: boolean) {
...
export async function generateCashFlowExcel(storeId: string, month: number, year: number) {
  const store = await getStoreInfo(storeId)
...
export async function generateBalanceSheetExcel(storeId: string, month: number, year: number) {
  const store = await getStoreInfo(storeId)
  const locale = store?.locale || 'id'
  const startDate = new Date(year, month - 1, 1)
  const endDate = new Date(year, month, 0)
...
export async function generateTaxReportPPNExcel(storeId: string, month: number, year: number) {
  const store = await getStoreInfo(storeId)
...
export async function generateTaxReportPPHExcel(storeId: string, month: number, year: number) {
  const store = await getStoreInfo(storeId)
  ...
  wsData.push(['', ''])
  wsData.push(['10.', 'Beban Usaha', '', -(statement.operatingExpenses as any).actual || 0])
  wsData.push(['11.', 'Laba/(Rugi) Usaha', '', statement.revenue.totalSales - statement.costOfGoods.total - ((statement.operatingExpenses as any).actual || 0)])
  wsData.push(['', ''])
...
export async function generateMonthlyReportPackage(storeId: string, month: number, year: number) {
  const store = await getStoreInfo(storeId)
  const ppnRate = await getPpnRate(storeId)
  const startDate = new Date(year, month - 1, 1)
  const endDate = new Date(year, month, 0)
  ...
  cover.push(['4.', 'Total Beban Usaha', '', -(incomeStatement.operatingExpenses as any).actual || 0])
  ...
  isData.push(['4.1', 'Total Beban Usaha', '', -(incomeStatement.operatingExpenses as any).actual || 0])
  isData.push(['', ''])
