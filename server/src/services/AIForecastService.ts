...
import { subDays, startOfDay, isRamadan, isFriday, isHoliday } from '../utils/dateUtils'
...
function getIndonesiaMultiplier(date: Date): number {
  let multiplier = 1.0
...
async function getProductHistory(productId: string, days: number = 30) {
...
function calculateTrend(historicalData: { date: Date; quantity: number }[]): {
  average: number
...
export async function forecastSales(productId: string, storeId: string): Promise<ForecastResult> {
  ...
  if (history.length >= 100) confidence += 0.3
  else if (history.length >= 50) confidence += 0.2
  else if (history.length >= 20) confidence += 0.1

  // Reduce confidence if high trend volatility
  if (Math.abs(trend) > 20) confidence -= 0.1
...
export async function generateRestockSuggestions(storeId: string): Promise<RestockSuggestion[]> {
...
export async function predictWaste(storeId: string) {
  const inventory = await prisma.inventory.findMany({
    where: { storeId }
  })
...
export async function getSalesComparison(storeId: string) {
