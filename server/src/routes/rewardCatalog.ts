...
router.get('/', authenticate, authorize('admin', 'manager'), async (req: AuthRequest, res) => {
  try {
    ...
    const rewards = active === 'true'
      ? await getActiveRewards(storeId)
      : await getRewards(storeId)
...
router.post('/', authenticate, authorize('admin'), async (req: AuthRequest, res) => {
  try {
    const storeId = req.user!.storeId
    const reward = await createReward({ ...req.body, storeId })
...
router.post('/:id/redeem', authenticate, authorize('admin', 'manager'), async (req: AuthRequest, res) => {
  try {
    const { memberId, orderId } = req.body
    if (!memberId) {
      return res.status(400).json({ code: 400, message: 'memberId is required' })
    }
    const memberReward = await redeemReward(memberId, req.params.id, orderId)
...
router.get('/member/:memberId', authenticate, authorize('admin', 'manager'), async (req: AuthRequest, res) => {
  try {
    const { unused } = req.query
    const rewards = unused === 'true'
      ? await getMemberUnusedRewards(req.params.memberId)
      : await getMemberRewards(req.params.memberId)
    res.json({ code: 200, data: { list: rewards }, timestamp: new Date().toISOString() })
  } catch (error) {
...
router.put('/use/:id', authenticate, authorize('admin', 'manager'), async (req: AuthRequest, res) => {
  try {
    const { orderId } = req.body
    const memberReward = await useReward(req.params.id, orderId)
...
export { router as rewardCatalogRouter }
