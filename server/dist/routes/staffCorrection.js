...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const correction = await database_1.prisma.attendanceCorrection.create({
            data: {
                staffId,
                storeId,
                date: new Date(date),
...
router.get('/my', auth_1.authenticate, async (req, res) => {
    try {
        const staffId = req.user.staffId;
        const corrections = await database_1.prisma.attendanceCorrection.findMany({
            where: { staffId },
...
router.put('/:id/approve', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
        const { adminNote } = req.body;
        const processedBy = req.user.id;
        const correction = await database_1.prisma.attendanceCorrection.findUnique({ where: { id } });
        if (!correction) {
            return res.status(404).json({ code: 404, message: 'Correction not found' });
        }
        // 更新纠错申请状态
        const updated = await database_1.prisma.attendanceCorrection.update({
            where: { id },
            data: {
                status: 'approved',
                adminNote,
...
router.put('/:id/reject', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
        const { adminNote } = req.body;
        const processedBy = req.user.id;
...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const staffId = req.user.staffId;
        const storeId = req.user.storeId;
        const swap = await database_1.prisma.shiftSwap.create({
            data: {
                staffId,
                storeId,
...
router.get('/my', auth_1.authenticate, async (req, res) => {
    try {
        const staffId = req.user.staffId;
        const swaps = await database_1.prisma.shiftSwap.findMany({
            where: { staffId },
            orderBy: { createdAt: 'desc' }
        });
        res.json({
...
router.put('/:id/approve', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        ...
        const processedBy = req.user.id;
        const storeId = req.user.storeId;
        const swap = await database_1.prisma.shiftSwap.findUnique({ where: { id } });
        if (!swap) {
            return res.status(404).json({ code: 404, message: 'Shift swap not found' });
        }
        // Check if confirmation is required
        const confirmationRequired = await (0, StaffConfigService_1.isFeatureEnabled)(storeId, 'shiftSwapConfirmation');
        ...
        // 更新调班申请状态
        const updated = await database_1.prisma.shiftSwap.update({
            where: { id },
            data: {
                status: 'approved',
                adminNote,
        ...
        await database_1.prisma.schedule.updateMany({
            where: {
...
router.put('/:id/confirm', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const staffId = req.user.staffId;
        const swap = await database_1.prisma.shiftSwap.findUnique({ where: { id } });
...
router.get('/pending-confirm', auth_1.authenticate, async (req, res) => {
    try {
        const staffId = req.user.staffId;
        const swaps = await database_1.prisma.shiftSwap.findMany({
            where: {
...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const overtime = await database_1.prisma.overtimeRequest.create({
            data: {
                staffId,
                storeId,
                date: new Date(date),
...
router.get('/my', auth_1.authenticate, async (req, res) => {
    try {
        const staffId = req.user.staffId;
        const requests = await database_1.prisma.overtimeRequest.findMany({
            where: { staffId },
            orderBy: { createdAt: 'desc' }
        });
        res.json({
...
router.put('/:id/approve', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        ...
        const { adminNote } = req.body;
        const processedBy = req.user.id;
        const updated = await database_1.prisma.overtimeRequest.update({
            where: { id },
            data: {
                status: 'approved',
                adminNote,
...
//# sourceMappingURL=staffCorrection.js.map