...
exports.productPriceRouter = void 0;
const express_1 = require("express");
const auth_1 = require("../middlewares/auth");
const database_1 = __importDefault(require("../config/database"));
const router = (0, express_1.Router)();
exports.productPriceRouter = router;
// GET /api/products/:productId/price - 获取商品在指定渠道的价格
router.get('/:productId/price', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (channelId && typeof channelId === 'string') {
            ...
            if (channelPrice && channelPrice.enabled) {
                ...
                channelName = (await database_1.default.channel.findUnique({ where: { id: channelId } }))?.name || '';
            }
            else {
                // 没有自定义价格，检查渠道的默认佣金
...
router.get('/:productId/channels', auth_1.authenticate, async (req, res) => {
    try {
        const { productId } = req.params;
        // 获取商品和所有渠道
        const [product, channels] = await Promise.all([
            database_1.default.product.findUnique({
                where: { id: productId },
                include: { specs: true }
            }),
            database_1.default.channel.findMany({
        ...
        // 获取商品在所有渠道的自定义价格
        const customPrices = await database_1.default.productChannelPrice.findMany({
            where: { productId }
        });
...
router.put('/:productId/channels/:channelId', auth_1.authenticate, async (req, res) => {
    try {
        const { productId, channelId } = req.params;
        const { priceAdjustment, enabled } = req.body;
        const productChannelPrice = await database_1.default.productChannelPrice.upsert({
            where: {
                productId_channelId: { productId, channelId }
            },
            create: {
