...
router.get('/hourly', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { storeId, date } = req.query;
        const where = {
            status: 'completed',
            createdAt: {
                gte: new Date(date || new Date().toISOString().slice(0, 10)),
                lt: new Date(new Date(date || new Date().toISOString().slice(0, 10)).getTime() + 24 * 60 * 60 * 1000)
            }
        };
        if (storeId)
            where.storeId = storeId;
        const orders = await database_1.default.order.findMany({ where });
        const hourlyStats = [];
        for (let i = 0; i < 24; i++) {
...
router.get('/inventory', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
...
router.get('/staff', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { storeId, month } = req.query;
        const where = {};
        if (storeId)
            where.storeId = storeId;
        const staff = await database_1.default.staff.findMany({
            where,
            include: {
...
router.get('/dashboard', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        ...
        // ============ 今日数据 ============
        const todayOrders = await database_1.default.order.findMany({
            where: {
                storeId,
                status: 'completed',
        ...
        // ============ 昨日数据（对比）============
        const yesterdayOrders = await database_1.default.order.findMany({
            where: {
                storeId,
                status: 'completed',
        ...
        // ============ 本月数据 ============
        const monthOrders = await database_1.default.order.findMany({
            where: {
                storeId,
                status: 'completed',
        ...
        // ============ 库存预警 ============
        const allInventory = await database_1.default.inventory.findMany({
            where: { storeId }
        });
        ...
        const todayAttendance = await database_1.default.attendance.count({
            where: {
                checkInTime: { gte: todayUTC }
            }
        });
        // 待审批请假
        const pendingLeave = await database_1.default.leave.count({
            where: {
                storeId,
                status: 'pending'
            }
        ...
        // ============ 会员情况 ============
        const todayNewMembers = await database_1.default.member.count({
            where: {
                storeId,
                createdAt: { gte: todayUTC }
            }
        ...
        const topProducts = Object.entries(productStats)
            ...
            .slice(0, 5);
        // ============ 月度目标进度 ============
        const revenueGoal = await database_1.default.config.findFirst({
...
router.get('/channels/:id', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
        const { startDate, endDate } = req.query;
        const channel = await database_1.default.channel.findUnique({ where: { id } });
        if (!channel) {
            return res.status(404).json({ code: 404, message: 'Channel not found' });
        }
        const where = { channelId: id, status: 'completed' };
        if (startDate || endDate) {
            where.createdAt = {};
            if (startDate)
                where.createdAt.gte = new Date(startDate);
...
router.get('/commissions', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { storeId, startDate, endDate } = req.query;
        const where = {};
        if (storeId)
