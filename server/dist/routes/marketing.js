...
const createCampaignSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['birthday', 'reactivation', 'loyalty', 'seasonal', 'welcome', 'points_expiring']),
    triggerType: zod_1.z.enum(['automatic', 'manual', 'scheduled']),
    startDate: zod_1.z.string(),
    endDate: zod_1.z.string().optional(),
    conditions: zod_1.z.object({
        thresholdDays: zod_1.z.number().optional(),
        minPoints: zod_1.z.number().optional()
    }).optional(),
    actions: zod_1.z.object({
        couponId: zod_1.z.string().optional(),
        notificationMessage: zod_1.z.string().optional()
    }).optional()
});
const createCouponSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['discount_percent', 'discount_fixed', 'free_product', 'free_delivery']),
    value: zod_1.z.number().int().positive(),
    minOrder: zod_1.z.number().int().optional().default(0),
    maxDiscount: zod_1.z.number().int().optional().default(0),
    validFrom: zod_1.z.string(),
    validUntil: zod_1.z.string(),
    usageLimit: zod_1.z.number().int().optional().default(0)
});
// ==================== CAMPAIGNS ====================
// GET /api/marketing/campaigns
router.get('/campaigns', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
...
router.post('/campaigns', auth_1.authenticate, (0, auth_1.authorize)('admin'), (0, validation_1.validateBody)(createCampaignSchema), async (req, res) => {
    try {
        // Always use user's storeId for security
...
router.put('/campaigns/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/campaigns/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/campaigns/:id/stats', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/coupons', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/coupons', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), (0, validation_1.validateBody)(createCouponSchema), async (req, res) => {
    try {
...
router.get('/coupons/:id', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.put('/coupons/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/coupons/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/coupons/:couponId/generate/:memberId', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
...
router.post('/coupons/:memberCouponId/redeem', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/automation/run', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/automation/birthday', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/automation/reactivation', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/automation/points-expiring', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/automation/seasonal', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.post('/automation/run/:campaignId', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
...
router.get('/automation/time', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const time = await (0, MarketingSchedulerService_1.getAutomationTime)(storeId);
        res.json({
            code: 200,
...
router.put('/automation/time', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { time } = req.body;
        ...
        await (0, MarketingSchedulerService_1.setAutomationTime)(storeId, time);
        res.json({
            code: 200,
...
router.get('/members/:id/balance', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const logs = await database_1.default.memberBalanceLog.findMany({
            where: { memberId: id },
...
router.post('/members/:id/balance/topup', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const [updatedMember, log] = await database_1.default.$transaction([
            database_1.default.member.update({
                where: { id },
                data: { balance: balanceAfter }
            }),
            database_1.default.memberBalanceLog.create({
                data: {
                    memberId: id,
                    type: 'topup',
                    amount,
...
router.post('/members/:id/balance/deduct', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const { id } = req.params;
        const { amount, note } = req.body;
        if (!amount || typeof amount !== 'number' || amount <= 0) {
...
router.get('/balance-logs', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const logs = await database_1.default.memberBalanceLog.findMany({
...
router.get('/discount-rules', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const rules = await database_1.default.discountRule.findMany({
            where: { storeId },
...
router.post('/discount-rules', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        const rule = await database_1.default.discountRule.create({
            data: {
                storeId,
                name,
                minOrderAmount,
...
router.put('/discount-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, minOrderAmount, discountValue, discountType, maxDiscount, applicableChannels, validFrom, validUntil, priority, status } = req.body;
        const existing = await database_1.default.discountRule.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
            return res.status(404).json({ code: 404, message: 'Discount rule not found' });
        }
        const rule = await database_1.default.discountRule.update({
            where: { id },
            data: {
                name,
                minOrderAmount,
...
router.delete('/discount-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await database_1.default.discountRule.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
        ...
        await database_1.default.discountRule.delete({ where: { id } });
        res.json({
            code: 200,
...
router.get('/timed-specials', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const specials = await database_1.default.timedSpecial.findMany({
            where: { storeId },
...
router.post('/timed-specials', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        const special = await database_1.default.timedSpecial.create({
            data: {
                storeId,
                name,
                productId,
...
router.put('/timed-specials/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, productId, specialPrice, originalPrice, startTime, endTime, daysOfWeek, applicableChannels, status } = req.body;
        const existing = await database_1.default.timedSpecial.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
            return res.status(404).json({ code: 404, message: 'Timed special not found' });
        }
        const special = await database_1.default.timedSpecial.update({
            where: { id },
            data: {
                name,
                productId,
...
router.delete('/timed-specials/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await database_1.default.timedSpecial.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
        ...
        await database_1.default.timedSpecial.delete({ where: { id } });
        res.json({
            code: 200,
...
router.get('/stacking-rules', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const rules = await database_1.default.stackingRule.findMany({
            where: { storeId },
            orderBy: { priority: 'desc' }
        });
        res.json({
...
router.post('/stacking-rules', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        const rule = await database_1.default.stackingRule.create({
            data: {
                storeId,
                name,
                ruleType,
...
router.put('/stacking-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, ruleType, couponTypeA, couponTypeB, priority, note, status } = req.body;
        const existing = await database_1.default.stackingRule.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
            return res.status(404).json({ code: 404, message: 'Stacking rule not found' });
        }
        const rule = await database_1.default.stackingRule.update({
            where: { id },
            data: {
                name,
                ruleType,
...
router.delete('/stacking-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await database_1.default.stackingRule.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
        ...
        await database_1.default.stackingRule.delete({ where: { id } });
        res.json({
            code: 200,
...
router.get('/automation-rules', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const rules = await database_1.default.campaign.findMany({
            where: {
                storeId,
...
router.post('/automation-rules', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        const rule = await database_1.default.campaign.create({
            data: {
                storeId,
                name,
                type: type || 'welcome',
...
router.put('/automation-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, type, triggerType, startDate, endDate, status, actions } = req.body;
        ...
        const rule = await database_1.default.campaign.update({
            where: { id },
            data: {
                name,
                type,
...
router.put('/automation-rules/:id/status', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/automation-rules/:id', auth_1.authenticate, (0, auth_1.authorize)('admin'), async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await database_1.default.campaign.findUnique({ where: { id } });
        if (!existing || existing.storeId !== req.user.storeId) {
        ...
        await database_1.default.campaign.delete({ where: { id } });
        res.json({
            code: 200,
...
router.get('/automation-logs', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const logs = await database_1.default.automationLog.findMany({
            where: { storeId },
...
router.get('/message-stats', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        ...
        const logs = await database_1.default.messageLog.findMany({
            where: {
                storeId,
...
router.get('/coupon-reports', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.get('/campaign-reports', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
...
router.get('/referral-funnel', auth_1.authenticate, (0, auth_1.authorize)('admin', 'manager'), async (req, res) => {
    try {
        const storeId = req.user.storeId;
        const { period = 'month' } = req.query;
        // Calculate date range
        ...
        // Get referral logs
        const referralLogs = await database_1.default.referralLog.findMany({
            where: {
        ...
        const referredMembers = await database_1.default.member.findMany({
