...
exports.getMemberReport = getMemberReport;
const database_1 = __importDefault(require("../config/database"));
const dateUtils_1 = require("../utils/dateUtils");
const BomService_1 = require("./BomService");
const InventoryService_1 = require("./InventoryService");
// Dashboard summary
async function getDashboardSummary(storeId, date) {
    const targetDate = date || new Date();
    const todayStart = (0, dateUtils_1.startOfDay)(targetDate);
    const todayEnd = (0, dateUtils_1.endOfDay)(targetDate);
    const monthStart = (0, dateUtils_1.startOfMonth)(targetDate);
    const monthEnd = (0, dateUtils_1.endOfMonth)(targetDate);
    // Get last 30 days for analysis
    const analysisStart = (0, dateUtils_1.startOfDay)((0, dateUtils_1.subDays)(targetDate, 30));
    const analysisEnd = (0, dateUtils_1.endOfDay)(targetDate);
    const [todayOrders, monthOrders, todayStaff, lowStockAlerts, consumptionAnomalies] = await Promise.all([
        ...
        (0, BomService_1.getLowStockAlerts)(storeId, 7),
        // Consumption anomalies (variance > 10%)
        (0, InventoryService_1.getConsumptionAnalysis)({
            storeId,
...
// Sales report
async function getSalesReport(storeId, dateRange) {
...
async function getStaffReport(storeId, dateRange) {
    const orders = await database_1.default.order.findMany({
        where: {
            storeId,
            createdAt: { gte: dateRange.startDate, lte: dateRange.endDate },
    ...
    const staffMembers = await database_1.default.staff.findMany({
...
// Inventory report
async function getInventoryReport(storeId) {
    ...
    // Calculate value
    const totalValue = inventory.reduce((sum, item) => sum + Number(item.currentStock) * Number(item.avgCost), 0);
...
// Member report
async function getMemberReport(storeId) {
    ...
    const inactiveMembers = members.filter(m => !m.lastVisit || new Date(m.lastVisit) < thirtyDaysAgo);
    return {
        summary: {
