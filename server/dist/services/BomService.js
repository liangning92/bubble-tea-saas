...
exports.getProductsWithBomCost = getProductsWithBomCost;
exports.getProductBomDetail = getProductBomDetail;
exports.getMaterialUsageForecast = getMaterialUsageForecast;
exports.getLowStockAlerts = getLowStockAlerts;
exports.calculateProductCost = calculateProductCost;
exports.getInventoryCostBreakdown = getInventoryCostBreakdown;
const database_1 = __importDefault(require("../config/database"));
/**
 * 获取所有产品的BOM成本分析
 */
async function getProductsWithBomCost(storeId) {
 ...
 * 获取单个产品的BOM明细
 */
async function getProductBomDetail(productId) {
    ...
    const bomDetails = await Promise.all(product.bomItems.map(async (item) => {
        ...
        const recipeUnit = (item.unit || inv?.unit || '个').toLowerCase();
 ...
 * 获取原料使用预测（根据历史销售）
 */
async function getMaterialUsageForecast(storeId, days = 30) {
    // 获取历史销售数据
 ...
 * 获取低库存预警（基于预测消耗）
 */
async function getLowStockAlerts(storeId, days = 7) {
 ...
 * 用于配方编辑页面显示每个原料的成本来源
 */
async function getInventoryCostBreakdown(inventoryId, quantity) {
