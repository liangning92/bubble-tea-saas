...
exports.seedDefaultReceiptTemplate = seedDefaultReceiptTemplate;
...
async function seedDefaultReceiptTemplate(storeId) {
    // Check if already exists
    const existing = await database_1.default.receiptTemplate.findFirst({
        where: { storeId }
    });
    if (existing) {
        return [];
