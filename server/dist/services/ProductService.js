...
exports.recalculateProductCost = recalculateProductCost;
exports.getProducts = getProducts;
exports.getProductById = getProductById;
exports.getProductByBarcode = getProductByBarcode;
exports.createProduct = createProduct;
exports.updateProduct = updateProduct;
exports.updateProductBom = updateProductBom;
exports.deleteProduct = deleteProduct;
exports.restoreProduct = restoreProduct;
exports.getProductsForPOS = getProductsForPOS;
exports.getLatestProductUpdate = getLatestProductUpdate;
exports.batchUpdateStatus = batchUpdateStatus;
exports.recalculateAllCosts = recalculateAllCosts;
exports.getProductCostDetail = getProductCostDetail;
...
function transformProductForApi(product) {
    ...
    return {
        ...
        specs: (specs || []).map((s) => serializeBigInt(s)),
        addons: (addons || []).map((a) => ({
            id: a.id,
            addonId: a.addonId,
            price: a.price,
        ...
        channelPrices: (channelPrices || []).map((cp) => serializeBigInt(cp)),
        bomItems: (bomItems || []).map((bi) => ({
            id: bi.id,
...
async function generateProductCode(storeId, categoryId) {
    // Get category info
    ...
    const count = await database_1.default.product.count({
...
// 单位换算系数
function getConversionFactor(inventoryUnit, recipeUnit) {
    const invUnit = (inventoryUnit || '').toLowerCase();
    const recUnit = (recipeUnit || '').toLowerCase();
    if (invUnit === recUnit)
        return 1;
    if (invUnit === 'kg') {
        if (recUnit === 'g' || recUnit === 'ml')
            return 1000;
    }
    if (invUnit === 'l') {
        if (recUnit === 'ml')
...
async function calculateProductCost(productId) {
    ...
    for (const item of bomItems) {
        ...
        if (inv.type === 'semi_finished' && inv.processRecipeId) {
            const breakdown = await (0, BomService_1.getInventoryCostBreakdown)(inv.id, item.quantity);
...
async function getProductByBarcode(barcode, storeId) {
    return database_1.default.product.findFirst({
        where: {
            storeId,
            OR: [
                { code: barcode },
...
async function updateProductBom(productId, bomItems) {
    // Delete existing BOM items
    await database_1.default.bOMItem.deleteMany({ where: { productId } });
    ...
    if (bomItems && bomItems.length > 0) {
        ...
        const inventories = await database_1.default.inventory.findMany({
            where: { id: { in: inventoryIds } }
        });
        const invMap = Object.fromEntries(inventories.map(i => [i.id, i]));
        await database_1.default.bOMItem.createMany({
...
async function getProductsForPOS(storeId) {
    const products = await database_1.default.product.findMany({
        where: {
            storeId,
            status: 'active',
    ...
    return products.map((p) => ({
        ...
        channelPrices: (p.channelPrices || []).map((cp) => ({
            channelId: cp.channelId,
            priceAdjustment: cp.priceAdjustment,
            enabled: cp.enabled
        }))
...
async function getLatestProductUpdate(storeId) {
    const result = await database_1.default.product.aggregate({
        where: {
            storeId,
            status: 'active',
            deletedAt: null
...
async function batchUpdateStatus(productIds, status) {
    return database_1.default.product.updateMany({
        where: { id: { in: productIds } },
        data: { status }
    });
}
// Recalculate all products cost
async function recalculateAllCosts(storeId) {
    const products = await database_1.default.product.findMany({
...
async function getProductCostDetail(productId) {
    const product = await database_1.default.product.findUnique({
        where: { id: productId },
        include: {
            specs: { orderBy: { isDefault: 'desc' } },
